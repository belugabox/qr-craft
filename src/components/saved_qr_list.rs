                ...
                for qr in saved.read().iter().cloned() {
                    {
                        ...
                        rsx! {
                            div {
                                ...
                                img { src: "data:image/png;base64,{qr.image_data}", width: "64", height: "64" }
                                ...
                                button {
                                    ...
                                    onclick: move |_| {
                                        let ui = ui.clone(); let qr_for_load = qr_for_load.clone(); to_owned![ui];
                                        async move {
                                            ui.set(UIQr { text: qr_for_load.text, size: qr_for_load.size, transparent: qr_for_load.transparent, image_data: format!("data:image/png;base64,{}", qr_for_load.image_data) });
                                        }
                                    },
                                    "Load"
                                }
                                button {
                                    ...
                                    onclick: move |_| {
                                        let saved = saved.clone(); let qr_for_delete = qr_for_delete.clone(); to_owned![saved];
                                        async move {
                                            if let Ok(_) = delete_saved(qr_for_delete.id).await {
                                                if let Ok(list) = list_saved().await {
                                                    saved.set(list);
                                                }
                                            }
