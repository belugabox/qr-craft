...
pub fn App() -> Element {
    ...
    rsx! {
        ...
        div { class: "flex flex-col min-h-screen bg-gray-900 text-white font-sans",
            ...
            header { class: "sticky z-50 top-0 p-4 bg-gray-900 flex justify-between items-center",
                h1 { class: "text-4xl font-bold text-teal-400", "{constants::APP_NAME}" }
                span { class: "text-xs text-gray-400", "v{env!(\"CARGO_PKG_VERSION\")}" }
            }
